...
pub mod doc_build {
	//! A module for generating documentation at build-time. Used internally by `lex` and
	//! `parse` as a part of their build process.

	// This is a workaround for the fact that cargo-doc doesn't let you include external
	// markdown files for documentation. The function 'build' takes the documentation at
	// 'doc', comments it with '//!', then concatenates it with the code in 'content'
	// writing the result to the file 'dest', allowing you to write the module
	// documentation for 'dest' in a seperate file. It also inserts a comment to the
	// begining to 'dest' telling the reader that the file is automatically generated, and
	// to edit 'doc' or 'content' instead. After the warning, but befor the documentation,
	// any strings in 'additional_headers' are written as-is. This allows you to insert
	...
	macro_rules! header {
		() => {
			r"// DO NOT EDIT THIS FILE. This file is automatically generated by 'build.rs', combining
// the module documentation at '{doc}' with the code from '{content}'.
//
// To edit the documentation for the top-level module of this crate, or to edit that
// module itself, edit the previous two documents, respectively.

"
		};
	}

	/// A function for combining documentation and content from seperate files. Used
	/// internally by `lex` and `parse` as a part of their build process.
	pub fn build<S>(doc: &str, content: &str, dest: &str, additional_headers: &[S]) -> Result<()>
	where
		S: Display,
	{
		let documentation = fs::read_to_string(doc)?;
		let code = fs::read_to_string(content)?;
		let mut dest_file = BufWriter::new(File::create(dest)?);
		let fout = &mut dest_file;
		write!(fout, header!(), doc = doc, content = content,)?;
		...
		fout.write_all(code.as_bytes())?;
		for filename in [doc, content] {
